...
public interface OrderProductDetailMapper {
     ...
     * @return
     */
    List<OrderDetailPageRes> detailPage(OrderDetailPageReq req);
    ...
    // 根据 IDs 删除多条记录
    int deleteByIdBatch(@Param("ids") List<Long> ids);
     ...
     * @return
     */
    List<OrderProductDetail> getProductListByOrderNo(@Param("factoryOrderNo") String factoryOrderNo);

    /**
     * 根据订单idList删除所有产品信息
     * @param orderIdList
     */
    void delOrderProductDetailByOrderIdList(@Param("orderIdList") List<Long> orderIdList);
